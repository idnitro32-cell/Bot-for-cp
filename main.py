...
vc_sessions:  dict[int, dict[int, float]]        = defaultdict(dict)
active_tickets: dict[int, dict] = {}  # Store active ticket info by channel ID
afk_users: dict[int, dict] = {}  # Store AFK users: {user_id: {"message": "reason", "time": timestamp}}
...
async def assign_roles(guild: discord.Guild, member: discord.Member, udata: dict, gdata: dict):
    ...
    # Check single-role conditional paths
    for path in s.get("conditional_paths", []):
        trigger_role_id = int(path["trigger_role_id"])
        trigger_role = guild.get_role(trigger_role_id)
        
        if trigger_role and trigger_role in member.roles:
            for target_role_id_str, required_msgs in path.get("thresholds", {}).items():
                target_role = guild.get_role(int(target_role_id_str))
                if target_role and total >= required_msgs and target_role not in member.roles:
                    try:
                        await member.add_roles(target_role, reason=f"Path trigger: {trigger_role.name} → {required_msgs} msgs")
                        log.info(f"[Path Role] {target_role.name} → {member} (via {trigger_role.name})")
                        newly_assigned_roles.append(target_role)
                    except:
                        pass
    
    # Check multi-role conditional paths
    for path_id, path_data in multi_role_paths.items():
        if path_data.get("guild_id") != guild.id:
            continue
        
        trigger_role_ids = set(path_data.get("trigger_role_ids", []))
        member_role_ids = {r.id for r in member.roles}
        
        # Check if member has ALL required trigger roles
        if trigger_role_ids.issubset(member_role_ids):
            for target_role_id_str, required_msgs in path_data.get("thresholds", {}).items():
                target_role = guild.get_role(int(target_role_id_str))
                if target_role and total >= required_msgs and target_role not in member.roles:
                    try:
                        await member.add_roles(target_role, reason=f"Multi-path {path_id}: {required_msgs} msgs")
                        log.info(f"[Multi-Path Role] {target_role.name} → {member} (Path ID: {path_id})")
                        newly_assigned_roles.append(target_role)
                    except:
                        pass
    ...
    def __init__(self, guild, user_id, bot_ref):
        ...
        for key, cat in HELP_CATEGORIES.items():
            ...
            if key == "ticket" and not is_admin:
                continue
            # ⬇️ ADD THIS NEW CONDITION ⬇️
            if key in ["ranks", "welcome", "settings", "milestones"] and not is_admin:
                continue
            options.append(
...
@bot.command(name="syncpaths")
async def prefix_syncpaths(ctx, path_type: str = "all"):
    """Sync roles for conditional paths and role thresholds
    Usage: !syncpaths [all|thresholds|paths]
    - all: Sync both message thresholds AND conditional paths (default)
    - thresholds: Sync only message role thresholds
    - paths: Sync only conditional role paths
    
    This will check ALL members and assign roles based on their current message counts.
    Useful for when you add new role thresholds or conditional paths after members
    have already reached the required message counts.
    """
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    
    if not is_owner(ctx.author, ctx.guild, gdata):
        return await ctx.send("❌ No permission. Admin or bot owner only.")
    
    path_type = path_type.lower()
    if path_type not in ["all", "thresholds", "paths"]:
        return await ctx.send("❌ Invalid option. Use: `all`, `thresholds`, or `paths`")
    
    await ctx.send("🔄 **Syncing roles...** This may take a moment for large servers.")
    
    # Send initial progress message
    status_msg = await ctx.send("📊 Starting sync process...")
    ...
    sync_thresholds = path_type in ["all", "thresholds"]
    sync_paths = path_type in ["all", "paths"]
    
    # Get settings
    role_thresholds = gdata["settings"].get("role_thresholds", {}) if sync_thresholds else {}
    conditional_paths = gdata["settings"].get("conditional_paths", []) if sync_paths else []
    
    if not role_thresholds and not conditional_paths:
        return await status_msg.edit(content="❌ No role thresholds or conditional paths found to sync!")
    
    # Create a dictionary to store which roles each member should have
    member_roles_to_add = {}
    ...
    for member in ctx.guild.members:
        ...
        if processed % 50 == 0:
            await status_msg.edit(content=f"📊 Processing members... ({processed}/{total_members})")
        
        # Get user data
        udata = get_user_data(gdata, member.id)
        total_msgs = udata.get("total", 0)
        
        roles_to_add = []
        
        # 1. Check default role thresholds
        if sync_thresholds and role_thresholds:
            # Get effective total (including VC if configured)
            include_vc = gdata["settings"].get("include_vc_in_roles", False)
            vc_total = udata.get("vc_total", 0)
            effective = total_msgs + (int(vc_total) if include_vc else 0)
            
            for role_id_str, required_msgs in role_thresholds.items():
                role = ctx.guild.get_role(int(role_id_str))
                if role and effective >= required_msgs and role not in member.roles:
                    roles_to_add.append(role)
        
        # 2. Check conditional paths
        if sync_paths and conditional_paths:
            for path in conditional_paths:
                trigger_role_id = int(path["trigger_role_id"])
                trigger_role = ctx.guild.get_role(trigger_role_id)
                
                # Check if member has the trigger role
                if trigger_role and trigger_role in member.roles:
                    # Apply thresholds from this path
                    for target_role_id_str, required_msgs in path.get("thresholds", {}).items():
                        target_role = ctx.guild.get_role(int(target_role_id_str))
                        if target_role and total_msgs >= required_msgs and target_role not in member.roles:
                            roles_to_add.append(target_role)
        
        # Store roles to add for this member (remove duplicates)
        if roles_to_add:
            member_roles_to_add[member.id] = list(set(roles_to_add))
    
    # Now assign the roles
    await status_msg.edit(content=f"✅ Finished scanning! Found {len(member_roles_to_add)} member(s) needing roles.\n📤 Assigning roles...")
    
    for member_id, roles in member_roles_to_add.items():
        ...
        try:
            for role in roles:
                await member.add_roles(role, reason="Role sync - already qualified")
                assigned_count += 1
                await asyncio.sleep(0.3)  # Small delay to avoid rate limits
        except Exception as e:
            errors += 1
            log.error(f"Failed to assign role to {member}: {e}")
    
    # Create result embed
    embed = discord.Embed(
        title="✅ Role Sync Completed",
        description=f"Successfully synced roles for **{ctx.guild.name}**",
        color=0x57F287,
        timestamp=datetime.utcnow()
    )
    
    embed.add_field(name="📊 Members Checked", value=f"**{total_members}**", inline=True)
    embed.add_field(name="🎭 Roles Assigned", value=f"**{assigned_count}**", inline=True)
    embed.add_field(name="⚠️ Errors", value=f"**{errors}**", inline=True)
    
    if sync_thresholds:
        embed.add_field(name="📋 Message Thresholds", value=f"`{len(role_thresholds)}` configured", inline=True)
    if sync_paths:
        embed.add_field(name="🔀 Conditional Paths", value=f"`{len(conditional_paths)}` configured", inline=True)
    
    embed.set_footer(text=f"Requested by {ctx.author.display_name}")
    
    await status_msg.edit(content=None, embed=embed)
    log.info(f"Role sync completed in {ctx.guild.id}: {assigned_count} roles assigned to {len(member_roles_to_add)} members")


@bot.command(name="syncvcroles")
async def prefix_syncvcroles(ctx):
    """Sync voice channel roles for all members
    This will check ALL members and assign VC roles based on their current voice time.
    Useful for when you add new VC role thresholds after members already have voice time.
    """
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if not vc_thresholds:
        return await ctx.send("❌ No VC role thresholds set! Use `!setvcrole @role <minutes>` first.")
    
    await ctx.send("🔄 **Syncing VC roles...** This may take a moment for large servers.")
    ...
    for member in ctx.guild.members:
        ...
        for role_id_str, required_minutes in vc_thresholds.items():
            role = ctx.guild.get_role(int(role_id_str))
            if role and vc_total >= required_minutes and role not in member.roles:
                roles_to_add.append(role)
        
        if roles_to_add:
            member_roles_to_add[member.id] = list(set(roles_to_add))
    
    await status_msg.edit(content=f"✅ Finished scanning! Found {len(member_roles_to_add)} member(s) needing VC roles.\n📤 Assigning roles...")
    ...
    for member_id, roles in member_roles_to_add.items():
        ...
        try:
            for role in roles:
                await member.add_roles(role, reason="VC role sync - already qualified")
                assigned_count += 1
                await asyncio.sleep(0.3)
        except Exception as e:
            errors += 1
            log.error(f"Failed to assign VC role to {member}: {e}")
    
    embed = discord.Embed(
        title="✅ VC Role Sync Completed",
        description=f"Successfully synced VC roles for **{ctx.guild.name}**",
        color=0x57F287,
        timestamp=datetime.utcnow()
    )
    
    embed.add_field(name="📊 Members Checked", value=f"**{total_members}**", inline=True)
    embed.add_field(name="🎭 VC Roles Assigned", value=f"**{assigned_count}**", inline=True)
    embed.add_field(name="⚠️ Errors", value=f"**{errors}**", inline=True)
    embed.add_field(name="🎙️ VC Thresholds", value=f"`{len(vc_thresholds)}` configured", inline=True)
    
    embed.set_footer(text=f"Requested by {ctx.author.display_name}")
    
    await status_msg.edit(content=None, embed=embed)
    log.info(f"VC role sync completed in {ctx.guild.id}: {assigned_count} roles assigned to {len(member_roles_to_add)} members")


@bot.command(name="syncall")
async def prefix_syncall(ctx):
    """Sync ALL roles (message thresholds, conditional paths, AND VC roles) at once"""
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    # Double confirmation for this heavy operation
    confirm_msg = await ctx.send(
        "⚠️ **WARNING** ⚠️\n"
        f"This will sync **ALL role types** (message thresholds, conditional paths, AND VC roles) for **{len([m for m in ctx.guild.members if not m.bot])} members**.\n\n"
        "This may take several minutes on large servers.\n\n"
    ...
    try:
        ...
        return await ctx.send("❌ Confirmation timed out. Sync cancelled.")
    
    await confirm_msg.delete()
    
    await ctx.send("🔄 **Syncing ALL roles...** This may take several minutes.")
    
    status_msg = await ctx.send("📊 Starting full sync process...")
    ...
    # Get all settings
    role_thresholds = gdata["settings"].get("role_thresholds", {})
    vc_thresholds = gdata["settings"].get("vc_role_thresholds", {})
    conditional_paths = gdata["settings"].get("conditional_paths", [])
    include_vc = gdata["settings"].get("include_vc_in_roles", False)
    ...
    for member_id, roles in member_roles_to_add.items():
        ...
        try:
            for role in roles:
                await member.add_roles(role, reason="Full role sync - already qualified")
                assigned_count += 1
    ...
    embed = discord.Embed(
        ...
        description=f"Successfully synced ALL roles for **{ctx.guild.name}**",
        color=0x57F287,
        timestamp=datetime.utcnow()
    )
    ...
    embed.add_field(name="🎭 Roles Assigned", value=f"**{assigned_count}**", inline=True)
    embed.add_field(name="⚠️ Errors", value=f"**{errors}**", inline=True)
    embed.add_field(name="📋 Message Thresholds", value=f"`{len(role_thresholds)}`", inline=True)
    embed.add_field(name="🎙️ VC Thresholds", value=f"`{len(vc_thresholds)}`", inline=True)
    embed.add_field(name="🔀 Conditional Paths", value=f"`{len(conditional_paths)}`", inline=True)
    
    embed.set_footer(text=f"Requested by {ctx.author.display_name}")
    
    await status_msg.edit(content=None, embed=embed)
    log.info(f"Full role sync completed in {ctx.guild.id}: {assigned_count} roles assigned to {len(member_roles_to_add)} members")
...
async def prefix_afk(ctx, *, reason: str = "AFK"):
    ...
    Usage: !afk [reason]
    Example: !afk at lunch, back in 30 minutes
    """
    ...
    # Get timeago format
    time_ago = f"<t:{int(current_time)}:R>"
    ...
    embed.add_field(name="⏰ Since", value=time_ago, inline=True)
    embed.set_thumbnail(url=ctx.author.display_avatar.url)
    embed.set_footer(text="You'll be automatically un-AFK when you send a message")
    
    await ctx.send(embed=embed)
    
    # Try to set nickname (requires manage_nicknames permission)
    try:
        new_nick = f"[AFK] {ctx.author.display_name[:20]}"
        await ctx.author.edit(nick=new_nick)
    except:
        pass  # Ignore if bot doesn't have permission


@bot.command(name="afkcheck")
async def prefix_afkcheck(ctx, member: discord.Member = None):
    ...
    if member.id in afk_users:
        afk_data = afk_users[member.id]
        time_ago = f"<t:{int(afk_data['time'])}:R>"
        
        embed = discord.Embed(
            title="🔴 AFK Status",
            description=f"**{member.display_name}** is currently AFK",
            color=0xFFA500,
            timestamp=datetime.utcnow()
        )
        embed.add_field(name="📝 Status", value=afk_data["message"], inline=False)
        embed.add_field(name="⏰ Since", value=time_ago, inline=True)
        embed.set_thumbnail(url=member.display_avatar.url)
...
async def prefix_afklist(ctx):
    """List all currently AFK users in the server"""
    if not afk_users:
        return await ctx.send("ℹ️ No users are currently AFK.")
    ...
    for user_id, data in afk_users.items():
        if data.get("guild_id") == ctx.guild.id:
            ...
            if member:
                time_ago = f"<t:{int(data['time'])}:R>"
                guild_afk.append(f"• {member.mention} — {data['message']} ({time_ago})")
    
    if not guild_afk:
        return await ctx.send("ℹ️ No users are currently AFK in this server.")
    ...
    embed = discord.Embed(
        title="🔴 AFK Users",
        description=afk_list[:2000],
        color=BOT_COLOR,
        timestamp=datetime.utcnow()
    )
    embed.set_footer(text=f"Total: {len(guild_afk)} user(s) AFK")
    await ctx.send(embed=embed)
...
async def on_message(message: discord.Message):
    ...
    if message.author.id in afk_users:
        ...
        # Remove AFK from nickname if it was set
        try:
            if message.author.nick and message.author.nick.startswith("[AFK]"):
                new_nick = message.author.nick.replace("[AFK] ", "").strip()
                await message.author.edit(nick=new_nick or None)
        except:
            pass
        
        embed = discord.Embed(
            title="🟢 AFK Mode Deactivated",
            description=f"Welcome back **{message.author.display_name}**! You were AFK for {afk_data['message']}",
            color=0x57F287,
            timestamp=datetime.utcnow()
        )
        ...
        for mentioned in message.mentions:
            if mentioned.id in afk_users and mentioned.id != message.author.id:
                ...
                try:
                    ...
                    dm_embed.add_field(name="📝 Status", value=afk_data["message"], inline=False)
                    dm_embed.add_field(name="⏰ Since", value=time_ago, inline=True)
                    dm_embed.add_field(
                        name="💬 Your Message",
                        value=f"```{message.content[:500]}```" if message.content else "*No text content*",
                        inline=False
                    )
                    dm_embed.set_footer(text=f"In #{message.channel.name}")
                    
                    await message.author.send(embed=dm_embed)
                except:
                    pass  # User has DMs disabled
                
                # Optional: Send a warning in the channel (can be removed if too spammy)
                warning_msg = await message.channel.send(
                    f"⚠️ **{mentioned.display_name}** is AFK: {afk_data['message']}\n"
                    f"*(They've been notified of your ping in DMs)*",
                    delete_after=10
                )
    
    # Rest of your existing on_message code continues here...
    data = get_data()
    gdata = get_guild_data(data, message.guild.id)
...
# ── MULTI-ROLE CONDITIONAL PATHS SYSTEM ─────────────────────────────────────────
# Store multi-role paths: {path_id: {"name": "name", "trigger_role_ids": [id1, id2], "thresholds": {role_id: count}}}
multi_role_paths: dict[int, dict] = {}

def load_multi_paths(data: dict):
    """Load multi-role paths from data"""
    global multi_role_paths
    multi_role_paths = data.get("_multi_role_paths", {})

def save_multi_paths(data: dict):
    """Save multi-role paths to data"""
    data["_multi_role_paths"] = multi_role_paths


@bot.command(name="addmultipath")
async def prefix_addmultipath(ctx, path_id: int, *trigger_roles: discord.Role):
    """Add a multi-role conditional path (requires ALL trigger roles)
    Usage: !addmultipath <unique_id> @Role1 @Role2 @Role3 ...
    Example: !addmultipath 1 @VIP @Moderator @Helper
    """
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if len(trigger_roles) < 2:
        return await ctx.send("❌ You need at least 2 trigger roles for a multi-role path!")
    
    if path_id in multi_role_paths:
        return await ctx.send(f"❌ Path ID `{path_id}` already exists! Use a different unique number or use `!removemultipath {path_id}` first.")
    
    # Store the path
    multi_role_paths[path_id] = {
        "name": f"Multi-Path {path_id}",
        "trigger_role_ids": [role.id for role in trigger_roles],
        "thresholds": {},
        "guild_id": ctx.guild.id
    }
    
    save_multi_paths(data)
    save_data(data)
    
    # Create display message
    trigger_mentions = " + ".join([role.mention for role in trigger_roles])
    
    embed = discord.Embed(
        title="🔗 Multi-Role Conditional Path Created",
        description=f"**Path ID:** `{path_id}`\n"
                    f"**Required Roles:** {trigger_mentions}\n"
                    f"**Status:** Awaiting role thresholds\n\n"
                    f"Use `!multipathedit {path_id} @TargetRole <message_count>` to add role thresholds.",
        color=0x9B59B6,
        timestamp=datetime.utcnow()
    )
    ...
    await ctx.send(embed=embed)
    log.info(f"Multi-role path {path_id} created by {ctx.author} with triggers: {[r.name for r in trigger_roles]}")


@bot.command(name="multipathedit")
async def prefix_multipathedit(ctx, path_id: int, target_role: discord.Role, message_count: int):
    """Add or edit a role threshold for a multi-role path
    Usage: !multipathedit <path_id> @TargetRole <message_count>
    Example: !multipathedit 1 @SuperVIP 1000
    """
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if path_id not in multi_role_paths:
        return await ctx.send(f"❌ Path ID `{path_id}` not found! Use `!listmultipaths` to see existing paths.")
    
    if message_count < 1:
        return await ctx.send("❌ Message count must be at least 1.")
    
    # Add or update threshold
    multi_role_paths[path_id]["thresholds"][str(target_role.id)] = message_count
    
    save_multi_paths(data)
    save_data(data)
    
    # Sort thresholds for display
    sorted_thresholds = sorted(multi_role_paths[path_id]["thresholds"].items(), key=lambda x: x[1])
    
    embed = discord.Embed(
        title="✅ Multi-Role Path Updated",
        description=f"**Path ID:** `{path_id}`\n"
                    f"**Target Role:** {target_role.mention}\n"
                    f"**Required Messages:** **{message_count:,}**\n\n"
                    f"**Current thresholds in this path:**",
        color=0x57F287,
        timestamp=datetime.utcnow()
    )
    
    threshold_text = ""
    for rid_str, req in sorted_thresholds:
        role = ctx.guild.get_role(int(rid_str))
        if role:
            threshold_text += f"• {role.mention} → `{req:,}` messages\n"
    
    embed.add_field(name="Role Thresholds", value=threshold_text or "None yet", inline=False)
    
    await ctx.send(embed=embed)
    log.info(f"Multi-role path {path_id} updated: {target_role.name} requires {message_count} messages")


@bot.command(name="multipathremove")
async def prefix_multipathremove(ctx, path_id: int, target_role: discord.Role = None):
    """Remove a threshold or entire multi-role path
    Usage: !multipathremove <path_id>              - Removes entire path
           !multipathremove <path_id> @TargetRole  - Removes only that role threshold
    """
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if target_role is None:
        ...
        confirm_msg = await ctx.send(
            f"⚠️ **WARNING** ⚠️\n"
            f"This will permanently delete multi-role path ID `{path_id}`.\n\n"
        ...
        del multi_role_paths[path_id]
        save_multi_paths(data)
        save_data(data)
        
        await ctx.send(f"✅ Multi-role path ID `{path_id}` has been permanently deleted.")
        log.info(f"Multi-role path {path_id} deleted by {ctx.author}")
    
    else:
        # Remove only specific threshold
        thresholds = multi_role_paths[path_id].get("thresholds", {})
        if str(target_role.id) not in thresholds:
            return await ctx.send(f"❌ Role {target_role.mention} not found in path ID `{path_id}`.")
        
        del thresholds[str(target_role.id)]
        save_multi_paths(data)
        save_data(data)
        
        await ctx.send(f"✅ Removed {target_role.mention} from multi-role path ID `{path_id}`.")
        log.info(f"Removed {target_role.name} from multi-role path {path_id} by {ctx.author}")


@bot.command(name="listmultipaths")
async def prefix_listmultipaths(ctx):
    """List all multi-role conditional paths"""
    if not multi_role_paths:
        return await ctx.send("ℹ️ No multi-role paths configured. Use `!addmultipath` to create one.")
    
    embed = discord.Embed(
        title="🔗 Multi-Role Conditional Paths",
        description="Paths that require ALL specified roles to unlock",
        color=0x9B59B6,
        timestamp=datetime.utcnow()
    )
    
    for path_id, path_data in multi_role_paths.items():
        if path_data.get("guild_id") != ctx.guild.id:
            continue
        
        # Get trigger roles
        trigger_roles = []
        for rid in path_data.get("trigger_role_ids", []):
            role = ctx.guild.get_role(rid)
            if role:
                trigger_roles.append(role.mention)
            else:
                trigger_roles.append(f"Deleted Role ({rid})")
        
        trigger_text = " + ".join(trigger_roles)
        
        # Get thresholds
        thresholds = path_data.get("thresholds", {})
        threshold_text = ""
        for rid_str, req in sorted(thresholds.items(), key=lambda x: x[1]):
            role = ctx.guild.get_role(int(rid_str))
            if role:
                threshold_text += f"  • {role.mention} → `{req:,}` msgs\n"
        
        if not threshold_text:
            threshold_text = "  *No thresholds set yet*"
        
        embed.add_field(
            name=f"📌 Path ID: `{path_id}`",
            value=f"**Requires:** {trigger_text}\n"
                  f"**Grants:**\n{threshold_text}",
            inline=False
        )
    
    embed.set_footer(text="Use !multipathedit <id> @role <count> to add thresholds")
    await ctx.send(embed=embed)

# Add this after the _data declaration or in the main execution block
# Load multi-role paths from data on startup
load_multi_paths(_data)

@bot.command(name="syncmultipaths")
async def prefix_syncmultipaths(ctx):
    """Sync ALL multi-role conditional paths - assign roles to members who qualify"""
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if not multi_role_paths:
        return await ctx.send("ℹ️ No multi-role paths configured.")
    
    # Filter paths for this guild
    guild_paths = {pid: pdata for pid, pdata in multi_role_paths.items() 
                   if pdata.get("guild_id") == ctx.guild.id}
    
    if not guild_paths:
        return await ctx.send("ℹ️ No multi-role paths found for this server.")
    
    await ctx.send("🔄 **Syncing multi-role paths...** This may take a moment.")
    status_msg = await ctx.send("📊 Starting sync process...")
    ...
    for member in ctx.guild.members:
        ...
        # Check each multi-role path
        for path_id, path_data in guild_paths.items():
            trigger_role_ids = set(path_data.get("trigger_role_ids", []))
    ...
    for member_id, roles in member_roles_to_add.items():
        ...
        try:
            for role in roles:
                await member.add_roles(role, reason="Multi-role path sync - already qualified")
                assigned_count += 1
                await asyncio.sleep(0.3)
        except Exception as e:
            errors += 1
    ...
    embed = discord.Embed(
        title="✅ Multi-Role Path Sync Completed",
        description=f"Successfully synced multi-role paths for **{ctx.guild.name}**",
        color=0x57F287,
        timestamp=datetime.utcnow()
    )
    ...
    embed.add_field(name="🎭 Roles Assigned", value=f"**{assigned_count}**", inline=True)
    embed.add_field(name="⚠️ Errors", value=f"**{errors}**", inline=True)
    embed.add_field(name="🔗 Active Paths", value=f"**{len(guild_paths)}**", inline=True)
    
    embed.set_footer(text=f"Requested by {ctx.author.display_name}")
    
    await status_msg.edit(content=None, embed=embed)
    log.info(f"Multi-role path sync completed in {ctx.guild.id}: {assigned_count} roles assigned")


@bot.command(name="multipathinfo")
async def prefix_multipathinfo(ctx, path_id: int):
    """Show detailed information about a specific multi-role path"""
    if path_id not in multi_role_paths:
        return await ctx.send(f"❌ Path ID `{path_id}` not found! Use `!listmultipaths` to see existing paths.")
    
    path_data = multi_role_paths[path_id]
    
    if path_data.get("guild_id") != ctx.guild.id:
        return await ctx.send(f"❌ Path ID `{path_id}` belongs to a different server.")
    
    embed = discord.Embed(
        title=f"🔗 Multi-Role Path Details - ID: {path_id}",
        color=0x9B59B6,
        timestamp=datetime.utcnow()
    )
    ...
    for rid in path_data.get("trigger_role_ids", []):
        role = ctx.guild.get_role(rid)
        if role:
            trigger_roles.append(f"{role.mention} (`{role.name}`)")
        else:
            trigger_roles.append(f"Deleted Role ({rid})")
    
    embed.add_field(
        name="📋 Required Roles (ALL needed)",
        value="\n".join(trigger_roles) or "None",
        inline=False
    )
    ...
    if thresholds:
        ...
        for rid_str, req in sorted(thresholds.items(), key=lambda x: x[1]):
            ...
            if role:
                threshold_text += f"• {role.mention} → **{req:,}** messages\n"
        embed.add_field(name="🎯 Role Thresholds", value=threshold_text, inline=False)
    else:
        embed.add_field(name="🎯 Role Thresholds", value="*No thresholds set yet*", inline=False)
    
    # Statistics
    total_eligible = 0
    member_role_ids_cache = {}
    
    for member in ctx.guild.members:
        if member.bot:
            continue
        member_role_ids_cache[member.id] = {r.id for r in member.roles}
    
    for member in ctx.guild.members:
        if member.bot:
            continue
        trigger_role_ids = set(path_data.get("trigger_role_ids", []))
        if trigger_role_ids.issubset(member_role_ids_cache.get(member.id, set())):
            total_eligible += 1
    
    embed.add_field(name="👥 Eligible Members", value=f"**{total_eligible}** members have all required roles", inline=False)
    
    await ctx.send(embed=embed)


@bot.command(name="syncsinglepaths")
async def prefix_syncsinglepaths(ctx):
    """Sync ONLY single-role conditional paths (standard !addpath paths)"""
    data = get_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if not conditional_paths:
        return await ctx.send("ℹ️ No single-role conditional paths configured. Use `!addpath` to create one.")
    
    await ctx.send("🔄 **Syncing single-role paths...** This may take a moment.")
    status_msg = await ctx.send("📊 Starting sync process...")
    ...
    for member_id, roles in member_roles_to_add.items():
        ...
        try:
            for role in roles:
                await member.add_roles(role, reason="Single-role path sync - already qualified")
                assigned_count += 1
                await asyncio.sleep(0.3)
        except Exception as e:
            errors += 1
    ...
    embed = discord.Embed(
        title="✅ Single-Role Path Sync Completed",
        description=f"Successfully synced single-role paths for **{ctx.guild.name}**",
        color=0x57F287,
        timestamp=datetime.utcnow()
    )
    ...
    embed.add_field(name="🎭 Roles Assigned", value=f"**{assigned_count}**", inline=True)
    embed.add_field(name="⚠️ Errors", value=f"**{errors}**", inline=True)
    embed.add_field(name="🔗 Active Paths", value=f"**{len(conditional_paths)}**", inline=True)
    
    await status_msg.edit(content=None, embed=embed)


# Update the assign_roles function to also check multi-role paths
# Find the existing assign_roles function and add this code inside it
# Look for the section that handles conditional paths and add this after it
