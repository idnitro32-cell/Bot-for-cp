...
def _default_settings() -> dict:
    return {
        # ... existing settings ...
        "announcement_channel": None,    # 👈 ADD THIS LINE
        "announcement_message": None,    # 👈 ADD THIS LINE
        # ... rest of settings ...
    }
...
async def assign_roles(guild: discord.Guild, member: discord.Member, udata: dict, gdata: dict):
    ...
    # Get announcement channel and custom message
    ann_channel_id = s.get("announcement_channel")
    ann_channel = guild.get_channel(int(ann_channel_id)) if ann_channel_id else None
    custom_msg = s.get("announcement_message")
    ...
    for rid_str, req in thresholds.items():
        ...
        if role and effective >= req and role not in member.roles:
            try:
                ...
                if ann_channel:
                    if custom_msg:
                        msg = custom_msg.replace("{mention}", member.mention)
                        msg = msg.replace("{role}", role.name)
                        msg = msg.replace("{count}", f"{req:,}")
                        ...
                        embed.add_field(name="📊 Message Count", value=f"{total:,}", inline=True)
                        embed.add_field(name="🏆 Milestone", value=f"{req:,} messages", inline=True)
    ...
    for rid_str, req_min in s.get("vc_role_thresholds", {}).items():
        ...
        if role and vc_total >= req_min and role not in member.roles:
            try:
                ...
                # Send VC announcement
                if ann_channel:
                    hours = int(req_min // 60)
                    mins = int(req_min % 60)
                    ...
                    if custom_msg:
                        msg = custom_msg.replace("{mention}", member.mention)
                        msg = msg.replace("{role}", role.name)
                        msg = msg.replace("{count}", time_str)
                        msg = msg.replace("{user}", member.display_name)
                        await ann_channel.send(msg)
                    else:
                        embed = discord.Embed(
                            title="🎙️ VOICE MILESTONE REACHED! 🎙️",
                            description=f"{member.mention} has earned **{role.name}**!",
                            color=role.color if role.color.value else discord.Color.purple(),
                            timestamp=datetime.utcnow()
                        )
                        embed.add_field(name="🎤 VC Time", value=f"{vc_total:.1f} minutes", inline=True)
                        embed.add_field(name="🏆 Milestone", value=time_str, inline=True)
                        embed.set_thumbnail(url=member.display_avatar.url)
                        embed.set_footer(text="Keep chatting in voice channels!")
                        await ann_channel.send(embed=embed)
...
async def prefix_setlogchannel(ctx, channel: discord.TextChannel):
 ...
 # ── Role Upgrade Announcements ───────────────────────────────────────────────────
@bot.command(name="setannounce")
async def prefix_setannounce(ctx, channel: discord.TextChannel):
    """Set channel for role upgrade announcements"""
    data = load_data()
    gdata = get_guild_data(data, ctx.guild.id)
    
    if not is_owner(ctx.author, ctx.guild, gdata):
        return await ctx.send("❌ Only server owners and admins can use this!")
    
    gdata["settings"]["announcement_channel"] = str(channel.id)
    save_data(data)
    
    embed = discord.Embed(
        title="📢 Role Upgrade Announcements Enabled",
        description=f"Role upgrades will be announced in {channel.mention}",
        color=discord.Color.green()
    )
    embed.set_footer(text="Use !removeannounce to disable")
    await channel.send(embed=embed)
    await ctx.send(f"✅ Role upgrade announcements set to {channel.mention}")

@bot.command(name="removeannounce")
async def prefix_removeannounce(ctx):
    """Disable role upgrade announcements"""
    data = load_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    old_channel_id = gdata["settings"].get("announcement_channel")
    if old_channel_id:
        old_channel = ctx.guild.get_channel(int(old_channel_id))
        if old_channel:
            await old_channel.send("🔕 Role upgrade announcements have been disabled.")
    
    gdata["settings"]["announcement_channel"] = None
    save_data(data)
    await ctx.send("✅ Role upgrade announcements disabled!")

@bot.command(name="announcechannel")
async def prefix_announcechannel(ctx):
    """Show current announcement channel"""
    data = load_data()
    gdata = get_guild_data(data, ctx.guild.id)
    
    channel_id = gdata["settings"].get("announcement_channel")
    
    if not channel_id:
        return await ctx.send("ℹ️ No announcement channel set. Use `!setannounce #channel` to set one.")
    
    channel = ctx.guild.get_channel(int(channel_id))
    if channel:
        await ctx.send(f"📢 Role upgrades are announced in {channel.mention}")
    else:
        await ctx.send("⚠️ Announcement channel set but not found! Use `!removeannounce` and set again.")

@bot.command(name="setannouncermsg")
async def prefix_setannouncermsg(ctx, *, message: str = None):
    """
    Set custom announcement message
    Placeholders: {mention}, {role}, {count}, {user}
    
    Example: !setannouncermsg "🎉 {mention} just got {role}! ({count} messages)"
    """
    data = load_data()
    gdata = get_guild_data(data, ctx.guild.id)
    ...
    if message is None:
        gdata["settings"]["announcement_message"] = None
        save_data(data)
        return await ctx.send("✅ Reset to default announcement message!")
    
    gdata["settings"]["announcement_message"] = message
    save_data(data)
    
    preview = message.replace("{mention}", "@User").replace("{role}", "@Role").replace("{count}", "1,000").replace("{user}", "Username")
    embed = discord.Embed(title="📢 Custom Announcement Set", description=f"Preview: {preview}", color=discord.Color.green())
    await ctx.send(embed=embed)
